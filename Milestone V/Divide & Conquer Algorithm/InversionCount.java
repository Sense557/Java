...
public class InversionCount {
    ...
    public static int mergeProcedure(int[] arr, int l, int mid, int h){
        ...
        //creation and copying the elements into the lsubarray and rsubarray 
        int[] lsubarray = Arrays.copyOfRange(arr, l, mid+1);
        int[] rsubarray = Arrays.copyOfRange(arr, mid+1, h+1);

        int i=0, j=0, k=l;

        //comparision left and right subarray
        ...
        // copy the remaining from the left subarray
        while(i < lsubarray.length){
            arr[k++] = lsubarray[i++];
        }

        // copy the remaining from the right subarray
